...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
        {
            ...
            std::string strUsage = _("People version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  ...
                  "  peopled [options] help                " + _("List commands") + "\n" +
                  "  peopled [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "people:"))
                fCommandLine = true;
...
int main(int argc, char* argv[])
{
    ...
    // Connect peopled signal handlers
    noui_connect();
