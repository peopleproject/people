...
#include "peoplebridge.moc"

PeopleBridge::PeopleBridge(PeopleGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void PeopleBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void PeopleBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void PeopleBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void PeopleBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void PeopleBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void PeopleBridge::populateOptions()
{
...
// Transactions
bool PeopleBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void PeopleBridge::clearRecipients()
{
    recipients.clear();
}

bool PeopleBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 MEN, 1 People
        switch(rcp.txnTypeInd)
        {
            case TXT_MEN_TO_MEN:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::MEN, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_MEN_TO_ANON:
                formatted.append(tr("<b>%1</b> to PEOPLE %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::MEN, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> PEOPLE, ring size %2 to PEOPLE %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::MEN, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_MEN:
                formatted.append(tr("<b>%1</b> PEOPLE, ring size %2 to MEN %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::MEN, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::MEN, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsPeopleBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your people balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::MEN, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void PeopleBridge::openCoinControl()
{
...
void PeopleBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void PeopleBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap PeopleBridge::listAnonOutputs()
{
...
void PeopleBridge::populateTransactionTable()
{
...
void PeopleBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void PeopleBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString PeopleBridge::transactionDetails(QString txid)
{
...
// Addresses
void PeopleBridge::populateAddressTable()
{
...
void PeopleBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void PeopleBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString PeopleBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString PeopleBridge::lastAddressError()
{
    QString sError;
...
QString PeopleBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void PeopleBridge::updateAddressLabel(QString address, QString label)
{
...
bool PeopleBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool PeopleBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void PeopleBridge::appendMessages(QVariantList messages, bool reset)
{
    emitMessages(messages, reset);
}

void PeopleBridge::appendMessage(int row)
{
...
void PeopleBridge::populateMessageTable()
{
...
void PeopleBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool PeopleBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool PeopleBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString PeopleBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);
}

QString PeopleBridge::addressForPubKey(QString pubkey)
{
    return addressModel->atm->addressForPubkey(pubkey);
}

bool PeopleBridge::setPubKey(QString address, QString pubkey)
{
...
bool PeopleBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
...
QString PeopleBridge::createGroupChat(QString label)
{
...
QString PeopleBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList PeopleBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void PeopleBridge::connectSignals()
{
...
QString PeopleBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap PeopleBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap PeopleBridge::listLatestBlocks()
{
...
QVariantMap PeopleBridge::findBlock(QString searchID)
{
...
QVariantMap PeopleBridge::blockDetails(QString blkHash)
{
...
QVariantMap PeopleBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap PeopleBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap PeopleBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap PeopleBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap PeopleBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap PeopleBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap PeopleBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap PeopleBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap PeopleBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with People prefix.");
            return result;
        }
...
QVariantMap PeopleBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap PeopleBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap PeopleBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
