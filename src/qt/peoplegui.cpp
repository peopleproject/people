...
PeopleGUI::PeopleGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new PeopleBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("Hpeople") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/people"));
    setWindowIcon(QIcon(":icons/people"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/people/index.html");
#else
    QFile html("/opt/people/index.html");
#endif
...
void PeopleGUI::pageLoaded(bool ok)
{
...
void PeopleGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void PeopleGUI::urlClicked(const QUrl & link)
{
...
void PeopleGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("Hpeople") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/people_testnet"));
            setWindowIcon(QIcon(":icons/people_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/people_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Hpeople client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/people_testnet"));
...
void PeopleGUI::setWalletModel(WalletModel *walletModel)
{
...
void PeopleGUI::setMessageModel(MessageModel *messageModel)
{
...
void PeopleGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Hpeople client"));
    trayIcon->setIcon(QIcon(":/icons/people"));
...
#ifndef Q_OS_MAC
void PeopleGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void PeopleGUI::setNumConnections(int count)
{
...
void PeopleGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void PeopleGUI::changeEvent(QEvent *e)
{
...
void PeopleGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void PeopleGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::MEN, nFeeRequired));
...
void PeopleGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void PeopleGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void PeopleGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void PeopleGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void PeopleGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void PeopleGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
void PeopleGUI::handleURI(QString strURI)
{
...
void PeopleGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void PeopleGUI::changePassphrase()
{
...
void PeopleGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void PeopleGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void PeopleGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void PeopleGUI::updateStakingIcon()
{
