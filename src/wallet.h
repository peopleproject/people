...
public:
    ...
    int64_t GetPeopleBalance() const;
    ...
    int64_t GetPeopleDebit(const CTxIn& txin) const;
    int64_t GetPeopleCredit(const CTxOut& txout) const;
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nMEN, int64_t& nPeople) const
    {
        nMEN = 0;
        nPeople = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nPeople += GetPeopleCredit(txout);
            } else
                nMEN += GetCredit(txout);
            if (!MoneyRange(nMEN)
                || !MoneyRange(nPeople))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
...
public:
    ...
    mutable bool fAvailablePeopleCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailablePeopleCreditCached;
    
    mutable int64_t nCredMENCached;
    mutable int64_t nCredPeopleCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailablePeopleCreditCached = false;
        ...
        nAvailablePeopleCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredMEN, int64_t& nCredPeople, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredMEN = nCredPeople = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredMENCached = 0;
            nCredPeopleCached = 0;
            pwallet->GetCredit(*this, nCredMENCached, nCredPeopleCached);
            fCreditSplitCached = true;
        };
        
        nCredMEN = nCredMENCached;
        nCredPeople = nCredPeopleCached;
        return true;
    }
    ...
    int64_t GetAvailablePeopleCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailablePeopleCreditCached)
            return nAvailablePeopleCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailablePeopleCredit() : value out of range");
            };
        };
        
        nAvailablePeopleCreditCached = nCredit;
        fAvailablePeopleCreditCached = true;
        return nCredit;
    };
